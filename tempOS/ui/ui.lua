...
function U.message(title,msg)
  local _,h=term.getSize(); U.clear(); U.header(title); local lines=U.wrap(msg,term.getSize()-4); for i,l in ipairs(lines) do if i<h-3 then term.setCursorPos(2,i+2); print(l) end end; U.footer("Press any key"); os.pullEvent("key")
end
