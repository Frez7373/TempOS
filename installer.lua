...
local function download(path) mkdirFor(path); local h,err=http.get(BASE..path,nil,true); if not h then return false,tostring(err) end; local data=h.readAll(); h.close(); if not data or #data==0 then return false,"empty response" end; local f=fs.open("/"..path,"w"); if not f then return false,"cannot open target" end; f.write(data); f.close(); return true end
...
local f=fs.open("/tempOS/data/installed","w"); if f then f.write("TempOS 1.0.0"); f.close() end
