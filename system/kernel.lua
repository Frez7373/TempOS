...
local function saveSettings() local f=fs.open(DATA,"w"); f.write(textutils.serialise(TempOS.settings)); f.close() end
_G.TempOS={version=VERSION,boot=os.epoch("utc"),settings=loadSettings(),events=E,process=Proc,network=N,devices=P}
...
TempOS.saveSettings=saveSettings
...
local screen,screenSide=chooseScreen(); TempOS.screen=screen; TempOS.screenSide=screenSide
term.redirect(screen)
P.scan(); N.init()
local apps={
  settings={name="Settings",icon="⚙",path="/apps/settings.lua"},
  filemanager={name="Files",icon="F",path="/apps/filemanager.lua"},
  terminal={name="Terminal",icon=">_",path="/apps/terminal.lua"},
  calculator={name="Calculator",icon="=",path="/apps/calculator.lua"},
  clock={name="Clock",icon="O",path="/apps/clock.lua"},
  systeminfo={name="System Info",icon="i",path="/apps/systeminfo.lua"},
  taskmanager={name="Task Manager",icon="T",path="/apps/taskmanager.lua"},
  editor={name="Text Editor",icon="E",path="/apps/editor.lua"},
  security={name="Security",icon="S",path="/apps/security.lua"},
  devices={name="Devices",icon="D",path="/apps/devices.lua"},
  network={name="Network",icon="N",path="/apps/network.lua"},
  updater={name="Updater",icon="U",path="/apps/updater.lua"}
}
TempOS.apps=apps
function TempOS.appLaunch(id)
  local a=apps[id]; if not a or not fs.exists(a.path) then return false,"application missing" end
  local ok,err=xpcall(function() dofile(a.path) end,debug.traceback)
  if not ok then TempOS.log("crash",id.."\n"..err); return false,err end
  return true
end
...
local Desktop=dofile("/ui/desktop.lua")
local ok,err=xpcall(function() Desktop.run(TempOS) end,debug.traceback)
