...
term.setTextColor(T.text); term.setBackgroundColor(T.bg); term.clear(); term.setCursorPos(1,1)
...
local function run(cmd,args)
  ...
  elseif cmd=="cd" then print("Current directory switching is handled by the shell service.")
  ...
  elseif cmd=="edit" and args[1] then shell.run("edit",args[1])
...
while true do
  write("tempos$ "); local line=read(); local parts={}
