...
while true do
  ...
  for i,s in ipairs(lines) do T.text(3,i+4,s,T.fg) end; T.text(2,h,"[P] ping broadcast  [R] rescan  [ESC] exit",T.muted)
  ...
  if e=="key" and a==keys.p then TempOS.network.broadcast({action="ping"}); T.text(3,math.max(1,h-2),"Ping sent",T.success); sleep(1) end
end
