...
while true do
  ...
  local tasks=TempOS.taskList(); for i,p in ipairs(tasks) do local s=string.format("%-5s %-20s %s",p.pid,p.name,p.status); T.text(2,i+4,s,T.text) end
  T.text(2,h,"[ESC] Exit   [K] stop selected",T.muted)
